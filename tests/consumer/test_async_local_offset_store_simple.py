...
class TestAsyncLocalOffsetStoreSimple(unittest.IsolatedAsyncioTestCase):
    """AsyncLocalOffsetStore 简化测试类"""

    async def asyncSetUp(self) -> None:
        """异步测试前准备"""
        # 创建临时目录用于测试
        self.temp_dir = tempfile.mkdtemp()
        self.consumer_group = "test_consumer_group_async_simple"

        # 创建AsyncLocalOffsetStore实例
    ...
    async def test_concurrent_memory_updates(self):
        ...
        self.assertEqual(offset1, 1004)  # 1000 + 4
        self.assertEqual(offset2, 2004)  # 2000 + 4
