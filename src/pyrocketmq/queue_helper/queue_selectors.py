...
class AsyncRoundRobinSelector(AsyncQueueSelector):
    ...
    async def areset_counter(self, topic: str) -> None:
        """异步重置指定主题的计数器"""
        async with self._lock:
            if topic in self._counters:
                del self._counters[topic]

    async def areset_all_counters(self) -> None:
        """异步重置所有计数器"""
        async with self._lock:
            self._counters.clear()
...
class AsyncRandomSelector(AsyncQueueSelector):
    ...
    async def aselect(
        ...
        # 为了避免阻塞事件循环，使用asyncio.sleep(0)让出控制权
...
class AsyncMessageHashSelector(AsyncQueueSelector):
    ...
    async def aselect(
        ...
        if not message:
            return await AsyncRandomSelector().aselect(topic, available_queues, message)

        # 获取分片键
        sharding_key = message.get_sharding_key()
        if not sharding_key:
            logger.debug(
                "No sharding key found for message, using async random selection"
            )
            return await AsyncRandomSelector().aselect(topic, available_queues, message)
...
# 异步便利函数
async def create_async_round_robin_selector() -> AsyncRoundRobinSelector:
    """创建异步轮询队列选择器"""
    # 为了避免阻塞事件循环，使用asyncio.sleep(0)让出控制权
    await asyncio.sleep(0)
...
async def create_async_random_selector() -> AsyncRandomSelector:
    """创建异步随机队列选择器"""
    await asyncio.sleep(0)
    return AsyncRandomSelector()


async def create_async_message_hash_selector() -> AsyncMessageHashSelector:
    """创建异步消息哈希队列选择器"""
    await asyncio.sleep(0)
    return AsyncMessageHashSelector()


# 直接返回异步选择器的便利函数（同步返回，使用时需要异步调用）
def get_async_round_robin_selector() -> AsyncRoundRobinSelector:
    """获取异步轮询队列选择器实例（同步返回）"""
    return AsyncRoundRobinSelector()


def get_async_random_selector() -> AsyncRandomSelector:
    """获取异步随机队列选择器实例（同步返回）"""
    return AsyncRandomSelector()


def get_async_message_hash_selector() -> AsyncMessageHashSelector:
    """获取异步消息哈希队列选择器实例（同步返回）"""
    return AsyncMessageHashSelector()
