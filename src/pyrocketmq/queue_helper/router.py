...
from pyrocketmq.model import BrokerData, Message, MessageQueue
from pyrocketmq.queue_helper.errors import (
    BrokerNotAvailableError,
    QueueNotAvailableError,
    RouteNotFoundError,
)
from pyrocketmq.queue_helper.queue_selectors import (
    AsyncQueueSelector,
    QueueSelector,
)
...
class AsyncMessageRouter:
    ...
    def __init__(
        ...
        # Topic-Broker映射
        self.topic_mapping: AsyncTopicBrokerMapping = topic_mapping
        ...
        # 预创建异步队列选择器（性能优化）
        self._async_selectors: dict[RoutingStrategy, AsyncQueueSelector] = {}
        self._create_async_selectors()

        self._logger.info(
            "AsyncMessageRouter initialized",
            extra={
                "strategy": default_strategy.value,
                "pre_created_selectors": list(self._async_selectors.keys()),
            },
        )
    ...
    async def aroute_message(
        ...
        # Debug: 记录异步路由请求开始
        self._logger.debug(
            "Starting async message routing",
            extra={
                "operation": "aroute_message",
                "topic": topic,
                "strategy": routing_strategy.value,
                "message_size": len(message.body) if message else 0,
                "message_properties": len(message.properties)
        ...
        try:
            ...
            available_queues: list[
                tuple[MessageQueue, BrokerData]
            ] = await self._aget_available_queues(topic)
            ...
            # 获取预创建的异步选择器
            selector = self._async_selectors.get(routing_strategy)
            if selector is None:
                # 这种情况不应该发生，因为我们预创建了所有策略
                error = ValueError(
                    f"Async selector not found for strategy: {routing_strategy.value}"
                )
                self._logger.error(
                    "Unexpected error: async selector not found",
                    extra={
                        "operation": "aroute_message",
                        "topic": topic,
                        "routing_strategy": routing_strategy.value,
                        "available_strategies": list(self._async_selectors.keys()),
                        "timestamp": time.time(),
                    },
                )
            ...
            # Debug: 记录异步选择器使用
            self._logger.debug(
                "Using pre-created async queue selector",
                extra={
                    "operation": "aroute_message",
                    "topic": topic,
                    "selector_type": type(selector).__name__,
            ...
            selected_result = await selector.aselect(topic, available_queues, message)
            ...
            # 异步选择Broker地址
            broker_address: str | None = await self._aselect_broker_address(broker_data)
            ...
            # Error: 记录路由失败的结构化日志
            self._logger.error(
                "Async routing failed with exception",
                extra={
                    "operation": "aroute_message",
                    "topic": topic,
                    "error_type": type(e).__name__,
    ...
    async def _aget_available_queues(
        self, topic: str
    ) -> list[tuple[MessageQueue, BrokerData]]:
        """异步获取Topic的可用队列列表"""
        return await self.topic_mapping.aget_available_queues(topic)

    def _create_async_selectors(self) -> None:
        """预创建所有异步队列选择器（性能优化）"""
        from pyrocketmq.queue_helper.queue_selectors import (
        ...
        # 创建所有可能用到的异步选择器
        self._async_selectors = {
            RoutingStrategy.ROUND_ROBIN: AsyncRoundRobinSelector(),
            RoutingStrategy.RANDOM: AsyncRandomSelector(),
            RoutingStrategy.MESSAGE_HASH: AsyncMessageHashSelector(),
        }

        self._logger.debug(
            "Pre-created async queue selectors",
            extra={
                "selectors_count": len(self._async_selectors),
                "selector_types": [
                    type(selector).__name__
                    for selector in self._async_selectors.values()
                ],
                "timestamp": time.time(),
            },
        )

    async def _aselect_broker_address(self, broker_data: BrokerData) -> str | None:
        """异步选择Broker地址"""
        # 这个方法主要是为了异步兼容性，实际地址选择是纯计算操作
        # 添加await asyncio.sleep(0)确保异步兼容性
    ...
    async def aselect_broker_address(self, broker_data: BrokerData) -> str | None:
        """异步选择Broker地址"""
        return await self._aselect_broker_address(broker_data)
...
def create_message_router(
    ...
    Args:
        topic_mapping: Topic-Broker映射管理器
...
def create_async_message_router(
    topic_mapping: AsyncTopicBrokerMapping,
    default_strategy: RoutingStrategy = RoutingStrategy.ROUND_ROBIN,
) -> AsyncMessageRouter:
    """
    ...
    Args:
        topic_mapping: 异步Topic-Broker映射管理器
        default_strategy: 默认路由策略

    Returns:
        AsyncMessageRouter: 异步消息路由器实例
    """
...
# 异步版本的便利函数
async def async_create_message_router(
    topic_mapping: AsyncTopicBrokerMapping,
    default_strategy: RoutingStrategy = RoutingStrategy.ROUND_ROBIN,
) -> AsyncMessageRouter:
    """
