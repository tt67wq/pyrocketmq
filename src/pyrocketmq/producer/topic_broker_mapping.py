...
class AsyncRouteInfo:
    ...
    def create_with_queues(
        ...
        Returns:
            AsyncRouteInfo: 包含预构建队列列表的AsyncRouteInfo实例
        """
        route_info: AsyncRouteInfo = cls(topic_route_data=topic_route_data)
...
# 便利函数
def create_topic_broker_mapping(route_timeout: float = 60.0) -> TopicBrokerMapping:
    """
    创建Topic-Broker映射管理器实例

    Args:
        route_timeout: 路由过期时间（秒）

    Returns:
        TopicBrokerMapping: 映射管理器实例
    """
    return TopicBrokerMapping(route_timeout=route_timeout)
...
class AsyncTopicBrokerMapping:
    ...
    专注于路由信息的缓存和管理，为异步MessageRouter提供基础数据支持。
    ...
    def __init__(self, route_timeout: float = 60.0) -> None:
        # 路由信息缓存: topic -> AsyncRouteInfo
        self._route_cache: dict[str, AsyncRouteInfo] = {}
        ...
        logger.info(
            "AsyncTopicBrokerMapping initialized",
            extra={
                "route_timeout": route_timeout,
            },
        )

    async def aget_route_info(self, topic: str) -> TopicRouteData | None:
        """
        异步获取Topic的路由信息
        ...
        async with self._lock:
            route_info: AsyncRouteInfo | None = self._route_cache.get(topic)
    ...
    async def aupdate_route_info(
        ...
        async with self._lock:
            try:
                # 创建新的路由信息并预先构建队列列表
                new_route_info: AsyncRouteInfo = AsyncRouteInfo.create_with_queues(
                    topic_route_data, topic
                )
    ...
    async def aget_available_queues(
        self, topic: str
    ) -> list[tuple[MessageQueue, BrokerData]]:
        """
        异步获取Topic的所有可用队列和对应的Broker（写队列）
    ...
    async def aget_subscribe_queues(
        ...
        """
        异步获取Topic的所有可订阅队列和对应的Broker（读队列）
    ...
    async def aget_available_brokers(self, topic: str) -> list[BrokerData]:
        """
        异步获取Topic的所有可用Broker
        ...
        Returns:
            list[BrokerData]: Broker列表
        """
        route_info: TopicRouteData | None = await self.aget_route_info(topic)
        if not route_info:
            return []

        return route_info.broker_data_list.copy()

    async def aget_queue_data(self, topic: str) -> list[QueueData]:
        """
        异步获取Topic的队列数据
    ...
    async def aget_all_topics(self) -> set[str]:
        """
        异步获取所有已缓存的Topic
    ...
    async def aforce_refresh(self, topic: str) -> bool:
        ...
        Returns:
            bool: 刷新是否成功
        """
        return await self.aremove_route_info(topic)

    def __str__(self) -> str:
        """字符串表示（同步方法）"""
        # 由于这个方法可能被异步上下文调用，我们使用同步方式获取统计信息
        # 这不会阻塞，因为只是获取基本属性
        total_topics = len(self._route_cache)
        return (
            f"AsyncTopicBrokerMapping(topics={total_topics}, "
...
def create_async_topic_broker_mapping(
    route_timeout: float = 60.0,
) -> AsyncTopicBrokerMapping:
    """
    创建异步Topic-Broker映射管理器实例

    Args:
        route_timeout: 路由过期时间（秒）

    Returns:
        AsyncTopicBrokerMapping: 异步映射管理器实例
    """
    return AsyncTopicBrokerMapping(route_timeout=route_timeout)


# 异步版本的便利函数
async def async_create_topic_broker_mapping(
    route_timeout: float = 60.0,
) -> AsyncTopicBrokerMapping:
    """
    异步创建异步Topic-Broker映射管理器实例
