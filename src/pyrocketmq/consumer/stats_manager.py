...
class AsyncStatsSnapshot:
    """异步统计快照数据结构"""

    def __init__(self, sum: int = 0, tps: float = 0.0, avgpt: float = 0.0):
        self.sum = sum
        self.tps = tps
        self.avgpt = avgpt

    def __repr__(self) -> str:
        return f"AsyncStatsSnapshot(sum={self.sum}, tps={self.tps:.2f}, avgpt={self.avgpt:.2f})"


class AsyncCallSnapshot:
    """异步调用快照数据结构"""

    def __init__(self, timestamp: int, times: int, value: int):
        self.timestamp = timestamp  # 毫秒时间戳
        self.times = times  # 调用次数
        self.value = value  # 累计值

    def __repr__(self) -> str:
        return f"AsyncCallSnapshot(timestamp={self.timestamp}, times={self.times}, value={self.value})"


class AsyncStatsItem:
    """异步版本的统计项。
    ...
    def __init__(self, stats_name: str, stats_key: str) -> None:
        ...
        # 不同时间窗口的数据存储
        self.minute_data: deque[AsyncCallSnapshot] = deque(
            maxlen=7
        )  # 分钟级数据，保留7个点
        self.hour_data: deque[AsyncCallSnapshot] = deque(
            maxlen=7
        )  # 小时级数据，保留7个点
        self.day_data: deque[AsyncCallSnapshot] = deque(
            maxlen=25
        )  # 天级数据，保留25个点

        self._lock: asyncio.Lock = asyncio.Lock()

    async def aadd_value(self, inc_value: int, inc_times: int) -> None:
        """异步增加统计值。
    ...
    async def asampling_in_seconds(self) -> None:
        """异步执行秒级采样。
    ...
    async def asampling_in_minutes(self) -> None:
        """异步执行分钟级采样。
    ...
    async def asampling_in_hour(self) -> None:
        """异步执行小时级采样。
    ...
    async def acompute_stats(
        self, data: deque[AsyncCallSnapshot]
    ) -> AsyncStatsSnapshot:
        """异步根据历史数据计算统计信息。
        ...
        Args:
            data (deque[AsyncCallSnapshot]): 包含历史快照数据的双端队列，至少需要2个数据点。

        Returns:
            AsyncStatsSnapshot: 包含累计值、TPS和平均处理时间的统计快照。
                当数据不足时返回空的统计快照（所有字段为默认值）。
        """
        if len(data) < 2:
            return AsyncStatsSnapshot()

        first: AsyncCallSnapshot = data[0]
        last: AsyncCallSnapshot = data[-1]
        ...
        return AsyncStatsSnapshot(sum=sum_value, tps=tps, avgpt=avgpt)

    async def aget_minute_stats(self) -> AsyncStatsSnapshot:
        """异步获取分钟级统计信息。
        ...
        Returns:
            AsyncStatsSnapshot: 分钟级统计快照，包含累计值、TPS和平均处理时间。
        """
        async with self._lock:
            return await self.acompute_stats(self.minute_data)

    async def aget_hour_stats(self) -> AsyncStatsSnapshot:
        """异步获取小时级统计信息。
        ...
        Returns:
            AsyncStatsSnapshot: 小时级统计快照，包含累计值、TPS和平均处理时间。
        """
        async with self._lock:
            return await self.acompute_stats(self.hour_data)

    async def aget_day_stats(self) -> AsyncStatsSnapshot:
        """异步获取天级统计信息。
        ...
        Returns:
            AsyncStatsSnapshot: 天级统计快照，包含累计值、TPS和平均处理时间。
        """
        async with self._lock:
            return await self.acompute_stats(self.day_data)


class AsyncStatsItemSet:
    """异步统计项集合。

    管理同一类型的多个统计项，提供自动化的数据采样功能。
    启动后台任务定期执行秒级、分钟级和小时级采样。
    """
    ...
    def __init__(self, stats_name: str) -> None:
        ...
        """
        self.stats_name: str = stats_name
        self.stats_items: dict[str, AsyncStatsItem] = {}
        self._lock: asyncio.Lock = asyncio.Lock()
        self._closed: bool = False
        self._sampling_task: asyncio.Task[None] | None = None
        self._minute_task: asyncio.Task[None] | None = None
        self._hour_task: asyncio.Task[None] | None = None
        # 注意：实际的采样任务需要在异步上下文中启动

    async def start_sampling(self) -> None:
        """异步启动采样任务。

        创建并启动三个后台任务分别执行不同频率的采样：
        - 秒级采样任务：每10秒执行一次
        - 分钟级采样任务：每10分钟执行一次
        - 小时级采样任务：每小时执行一次
        """

        async def sampling_worker() -> None:
            while not self._closed:
                await asyncio.sleep(10)  # 10秒采样一次
                if self._closed:
                    break
                await self._asampling_in_seconds()

        async def minute_worker() -> None:
            while not self._closed:
                await asyncio.sleep(600)  # 10分钟采样一次
                if self._closed:
                    break
                await self._asampling_in_minutes()

        async def hour_worker() -> None:
            while not self._closed:
                await asyncio.sleep(3600)  # 1小时采样一次
                if self._closed:
                    break
                await self._asampling_in_hour()

        # 启动采样任务
        self._sampling_task = asyncio.create_task(sampling_worker())
        self._minute_task = asyncio.create_task(minute_worker())
        self._hour_task = asyncio.create_task(hour_worker())

    async def _asampling_in_seconds(self) -> None:
        """异步对所有统计项执行秒级采样。
        ...
        async with self._lock:
            for item in self.stats_items.values():
                await item.asampling_in_seconds()

    async def _asampling_in_minutes(self) -> None:
        """异步对所有统计项执行分钟级采样。
        ...
        async with self._lock:
            for item in self.stats_items.values():
                await item.asampling_in_minutes()

    async def _asampling_in_hour(self) -> None:
        """异步对所有统计项执行小时级采样。
        ...
        async with self._lock:
            for item in self.stats_items.values():
                await item.asampling_in_hour()

    async def aadd_value(self, key: str, inc_value: int, inc_times: int) -> None:
        """异步添加统计值到指定的统计项。
        ...
        async with self._lock:
            if key not in self.stats_items:
                self.stats_items[key] = AsyncStatsItem(self.stats_name, key)
            await self.stats_items[key].aadd_value(inc_value, inc_times)

    async def aget_minute_stats(self, key: str) -> AsyncStatsSnapshot:
        """异步获取指定统计项的分钟级统计信息。
        ...
        Returns:
            AsyncStatsSnapshot: 分钟级统计快照。
                如果统计项不存在，返回空的统计快照（所有字段为默认值）。
        """
        async with self._lock:
            if key in self.stats_items:
                return await self.stats_items[key].aget_minute_stats()
            return AsyncStatsSnapshot()

    async def aget_hour_stats(self, key: str) -> AsyncStatsSnapshot:
        """异步获取指定统计项的小时级统计信息。
        ...
        Returns:
            AsyncStatsSnapshot: 小时级统计快照。
                如果统计项不存在，返回空的统计快照（所有字段为默认值）。
        """
        async with self._lock:
            if key in self.stats_items:
                return await self.stats_items[key].aget_hour_stats()
            return AsyncStatsSnapshot()
    ...
    async def ashutdown(self) -> None:
        ...
        if self._hour_task and not self._hour_task.done():
            self._hour_task.cancel()

        # 等待任务完成
        tasks_to_wait = [
            task
            for task in [self._sampling_task, self._minute_task, self._hour_task]
...
class AsyncStatsManager:
    ...
    def __init__(self) -> None:
        ...
        """
        # 初始化各种统计项集合
        self.consume_ok_tps: AsyncStatsItemSet = AsyncStatsItemSet("CONSUME_OK_TPS")
        self.consume_rt: AsyncStatsItemSet = AsyncStatsItemSet("CONSUME_RT")
        self.consume_failed_tps: AsyncStatsItemSet = AsyncStatsItemSet(
            "CONSUME_FAILED_TPS"
        )
        self.pull_tps: AsyncStatsItemSet = AsyncStatsItemSet("PULL_TPS")
        self.pull_rt: AsyncStatsItemSet = AsyncStatsItemSet("PULL_RT")
    ...
    async def astart(self) -> None:
        ...
        async with self._lock:
            ...
            await self.consume_ok_tps.start_sampling()
            await self.consume_rt.start_sampling()
            await self.consume_failed_tps.start_sampling()
            await self.pull_tps.start_sampling()
            await self.pull_rt.start_sampling()

            self._started = True

    def _make_key(self, group: str, topic: str) -> str:
        """生成统计键（同步方法，因为是纯字符串操作）。
    ...
    async def aincrease_pull_rt(self, group: str, topic: str, rt: int) -> None:
        """异步增加拉取响应时间统计。
        ...
        """
        key: str = self._make_key(group, topic)
        await self.pull_rt.aadd_value(key, rt, 1)

    async def aincrease_pull_tps(self, group: str, topic: str, msgs: int) -> None:
        """异步增加拉取TPS统计。
        ...
        """
        key: str = self._make_key(group, topic)
        await self.pull_tps.aadd_value(key, int(msgs), 1)

    async def aincrease_consume_rt(self, group: str, topic: str, rt: int) -> None:
        """异步增加消费响应时间统计。
        ...
        """
        key: str = self._make_key(group, topic)
        await self.consume_rt.aadd_value(key, rt, 1)

    async def aincrease_consume_ok_tps(self, group: str, topic: str, msgs: int) -> None:
        """异步增加消费成功TPS统计。
        ...
        """
        key: str = self._make_key(group, topic)
        await self.consume_ok_tps.aadd_value(key, int(msgs), 1)

    async def aincrease_consume_failed_tps(
        self, group: str, topic: str, msgs: int
    ) -> None:
        """异步增加消费失败TPS统计。
        ...
        """
        key: str = self._make_key(group, topic)
        await self.consume_failed_tps.aadd_value(key, int(msgs), 1)

    async def aget_consume_status(self, group: str, topic: str) -> ConsumeStatus:
        """异步获取指定消费者组和主题的消费状态。
        ...
        # 获取各项统计
        pull_rt_stats: AsyncStatsSnapshot = await self.pull_rt.aget_minute_stats(key)
        status.pull_rt = pull_rt_stats.avgpt

        pull_tps_stats: AsyncStatsSnapshot = await self.pull_tps.aget_minute_stats(key)
        status.pull_tps = pull_tps_stats.tps

        consume_rt_stats: AsyncStatsSnapshot = await self.consume_rt.aget_minute_stats(
            key
        )
        if consume_rt_stats.sum == 0:
            # 如果分钟级数据为空，使用小时级数据
            consume_rt_stats = await self.consume_rt.aget_hour_stats(key)
        status.consume_rt = consume_rt_stats.avgpt

        consume_ok_stats: AsyncStatsSnapshot = (
            await self.consume_ok_tps.aget_minute_stats(key)
        )
        status.consume_ok_tps = consume_ok_stats.tps

        consume_failed_stats: AsyncStatsSnapshot = (
            await self.consume_failed_tps.aget_minute_stats(key)
        )
        status.consume_failed_tps = consume_failed_stats.tps

        # 失败消息数使用小时级统计
        failed_hour_stats: AsyncStatsSnapshot = (
            await self.consume_failed_tps.aget_hour_stats(key)
        )
        status.consume_failed_msgs = failed_hour_stats.sum
    ...
    async def ashutdown(self) -> None:
        ...
        await asyncio.gather(
            self.consume_ok_tps.ashutdown(),
            self.consume_rt.ashutdown(),
            self.consume_failed_tps.ashutdown(),
            self.pull_tps.ashutdown(),
            self.pull_rt.ashutdown(),
            return_exceptions=True,
        )


# 异步示例使用代码
async def async_example_usage() -> None:
    """异步统计管理器使用示例。

    演示如何创建和使用异步统计管理器来收集RocketMQ消费者的性能数据。
    包括数据收集、采样等待和统计报告生成的完整流程。
    ...
    # 创建异步统计管理器
    async_stats_manager: AsyncStatsManager = AsyncStatsManager()

    # 启动统计管理器
    await async_stats_manager.astart()
    logger.info("异步统计管理器已启动")

    # 模拟数据收集
    group: str = "test_group"
    topic: str = "test_topic"

    logger.info("开始模拟异步统计数据收集...")

    # 模拟一些统计数据
    for i in range(100):
        await async_stats_manager.aincrease_pull_tps(group, topic, 10)
        await async_stats_manager.aincrease_pull_rt(group, topic, 50)
        await async_stats_manager.aincrease_consume_ok_tps(group, topic, 8)
        await async_stats_manager.aincrease_consume_rt(group, topic, 100)

        if i % 10 == 0:
            await async_stats_manager.aincrease_consume_failed_tps(group, topic, 1)

        await asyncio.sleep(0.1)  # 异步间隔100ms

    logger.info("等待采样完成...")
    await asyncio.sleep(15)  # 等待至少一次采样

    # 获取消费状态
    status: ConsumeStatus = await async_stats_manager.aget_consume_status(group, topic)

    logger.info("异步消费状态统计:")
    logger.info(f"  拉取响应时间: {status.pull_rt:.2f}ms")
    logger.info(f"  拉取TPS: {status.pull_tps:.2f}")
    logger.info(f"  消费响应时间: {status.consume_rt:.2f}ms")
    logger.info(f"  消费成功TPS: {status.consume_ok_tps:.2f}")
    ...
    # 关闭统计管理器
    await async_stats_manager.ashutdown()
    logger.info("异步统计管理器已关闭")
