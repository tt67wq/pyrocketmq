...
定义RocketMQ Consumer的异步消息处理回调接口。
提供灵活的异步消息处理机制，允许用户自定义异步消息消费逻辑。

异步版本与同步版本保持相同的API设计，只是所有回调方法都变成了异步方法，
使用async/await语法，适用于高并发异步应用场景。
"""
...
class AsyncConsumeContext:
    ...
    包含异步消息消费过程中的上下文信息，如队列信息、消费者组、
    消费次数等。这些信息可以帮助用户在异步消息处理时做出更合适的决策。

    与同步版本保持相同的结构，但提供了异步上下文管理能力。
    """
...
class AsyncMessageListener(ABC):
    ...
    定义异步消息处理的回调接口。用户需要继承这个类并实现
    consume_message方法来异步处理接收到的消息。

    设计原则:
    - 异步优先: 所有方法都是异步的，支持async/await
    - 简单易用: 提供清晰的异步接口定义
    - 灵活处理: 支持批量异步消息处理
    - 异常安全: 完善的异步错误处理机制
    - 可扩展性: 预留异步扩展点
    ...
    使用示例:
        >>> class MyAsyncMessageListener(AsyncMessageListener):
        ...     async def consume_message(self, messages, context):
        ...         for msg in messages:
        ...             # 异步处理消息
        ...             await self.process_message_async(msg)
    ...
    async def consume_message(
        ...
        当异步Consumer接收到消息时会调用这个方法。用户在这个方法中
        实现具体的异步消息处理逻辑。

        Args:
            messages: 接收到的消息列表(批量处理)
            context: 异步消费上下文，包含队列信息、消费组等元数据

        Returns:
            消费结果，决定消息如何被确认或重试

        Raises:
            Exception: 异步消息处理异常时会被Consumer捕获并处理
    ...
    async def on_success(
        self, _messages: list[MessageExt], _context: AsyncConsumeContext
    ) -> None:
        """
        异步消息处理成功回调(可选实现)

        当消息成功处理并被确认后异步调用，可以用于记录成功日志或
        执行其他异步后续操作。
    ...
    async def on_exception(
        ...
        _context: AsyncConsumeContext,
        _exception: Exception,
    ) -> None:
        """
        异步消息处理异常回调(可选实现)

        当异步消息处理过程中发生异常时调用，可以用于记录错误日志或
        执行异步异常恢复操作。
...
def create_async_message_listener(
    ...
    Examples:
        >>> # 创建简单异步监听器
        >>> async def handle_messages_async(messages):
        ...     for msg in messages:
        ...         await some_async_operation(msg)
        ...         print(f"异步处理: {msg.body}")
        ...     return ConsumeResult.SUCCESS
        >>>
