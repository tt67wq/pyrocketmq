...
class MessageListenerOrderly(MessageListener, ABC):
    """
    顺序消息监听器

    用于处理需要保证顺序的消息。RocketMQ会确保同一个队列中的消息
    按顺序投递给这个监听器，用户需要保证处理逻辑的线程安全。

    注意事项:
    - 必须保证处理逻辑的线程安全
    - 处理失败会影响后续消息的消费
    - 建议处理逻辑尽可能快速，避免阻塞后续消息
    """
    ...
    def consume_message(
        ...
        Note:
            由于是顺序消费，返回RECONSUME_LATER会阻塞后续消息
            处理，请谨慎使用。
        """
        logger.info(
            "开始顺序消费消息",
        ...
        try:
            # 调用用户实现的顺序消费逻辑
            return self.consume_message_orderly(messages, context)
        except Exception as e:
            logger.error(
                f"顺序消费消息失败: {e}",
                extra={
...
class MessageListenerConcurrently(MessageListener, ABC):
    """
    并发消息监听器

    用于处理并发消息。RocketMQ会并发地将消息投递给这个监听器，
    消息的顺序不保证，但消费效率更高。
    ...
    def consume_message(
        ...
        try:
            # 调用用户实现的并发消费逻辑
            return self.consume_message_concurrently(messages, context)
        except Exception as e:
            logger.error(
                f"并发消费消息失败: {e}",
                extra={
