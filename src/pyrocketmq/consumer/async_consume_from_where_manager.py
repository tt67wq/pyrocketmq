...
实现AsyncConsumeFromWhereManager策略管理，提供异步方式的消费起始位置查询功能。
负责根据不同的消费策略确定消费者开始消费的偏移量位置，支持从最后偏移量、第一个偏移量
或指定时间戳开始消费。所有网络操作都是异步的，适用于高并发异步应用场景。
"""
...
class AsyncConsumeFromWhereManager:
    ...
    负责根据不同的消费策略异步确定消费者开始消费的偏移量位置。
    通过AsyncBrokerManager与Broker通信获取队列的偏移量信息，并按照策略返回合适的起始偏移量。
    所有网络操作都是异步的，适用于高并发异步应用场景。
    ...
    def __init__(
        ...
        创建一个新的异步消费起始位置管理器，用于管理消费者组的消费起始位置策略。
        该管理器负责根据不同的消费策略异步确定消费者开始消费的偏移量位置。

        Args:
            consume_group (str): 消费者组名称，用于标识消费者的逻辑分组
            namesrv_manager (AsyncNameServerManager): 异步NameServer管理器实例，用于获取Broker地址信息
            broker_manager (AsyncBrokerManager): 异步Broker管理器实例，用于与Broker建立异步连接

        Raises:
            TypeError: 当任何参数的类型不正确时抛出
        """
        self.consume_group: str = consume_group
        self.namesrv_manager: AsyncNameServerManager = namesrv_manager
        self.broker_manager: AsyncBrokerManager = broker_manager
        self._logger: logging.Logger = get_logger(
            f"{__name__}.AsyncConsumeFromWhereManager"
        )

    async def get_consume_offset(
        self,
        queue: MessageQueue,
        strategy: str,
        timestamp: int = 0,
    ) -> int:
        """异步根据指定的消费策略获取队列的消费起始偏移量。

        根据不同的消费起始位置策略，异步计算出消费者应该从哪个偏移量开始消费消息。
        ...
        Examples:
            >>> manager = AsyncConsumeFromWhereManager(consume_group, namesrv_mgr, broker_mgr)
            >>> queue = MessageQueue(broker_name="broker1", topic="test_topic", queue_id=0)
            >>> offset = await manager.get_consume_offset(queue, ConsumeFromWhere.CONSUME_FROM_LAST_OFFSET)
            >>> print(f"从偏移量 {offset} 开始消费")
        """
        try:
            if strategy == ConsumeFromWhere.CONSUME_FROM_LAST_OFFSET:
                offset = await self._get_max_offset(queue)
                self._logger.info(
                ...
                offset = await self._get_min_offset(queue)
                self._logger.info(
                ...
                offset = await self._get_offset_by_timestamp(queue, timestamp)
                self._logger.info(
    ...
    async def _get_max_offset(self, queue: MessageQueue) -> int:
        """异步从Broker获取指定队列的最大偏移量。

        异步查询指定消息队列的最大偏移量，即队列中最后一条消息的下一条位置。
        ...
        """
        broker_address: str | None = await self.namesrv_manager.get_broker_address(
            queue.broker_name
        )
        ...
        try:
            pool: AsyncConnectionPool = await self.broker_manager.must_connection_pool(
                broker_address
            )
            async with pool.get_connection(usage="获取最大偏移量") as conn:
                broker_client: AsyncBrokerClient = AsyncBrokerClient(conn)
                max_offset: int = await broker_client.get_max_offset(
    ...
    async def _get_min_offset(self, queue: MessageQueue) -> int:
        """异步获取队列的最小偏移量。

        异步返回指定消息队列的最小偏移量，即队列中第一条消息的位置。
    ...
    async def _get_offset_by_timestamp(
        self, queue: MessageQueue, timestamp: int
    ) -> int:
        """异步根据时间戳从Broker获取对应的消息偏移量。

        异步查询指定时间戳在队列中对应的偏移量位置，用于实现从特定时间点开始消费。
        ...
        """
        broker_addr: str | None = await self.namesrv_manager.get_broker_address(
            queue.broker_name
        )
        if not broker_addr:
            raise BrokerNotAvailableError(
        ...
        try:
            ...
            async with pool.get_connection(usage="根据时间戳获取偏移量") as conn:
                broker_client: AsyncBrokerClient = AsyncBrokerClient(conn)
                offset: int = await broker_client.search_offset_by_timestamp(
                    topic=queue.topic,
    ...
    async def reset_offset_to_position(
        self,
        queue: MessageQueue,
        position: str,
        timestamp: int = 0,
        ...
        Example:
            ...
            >>>
            >>> # 从最新消息开始消费
            >>> offset = await manager.reset_offset_to_position(queue, "last")
            >>> print(f"从偏移量 {offset} 开始消费最新消息")
            >>>
            >>> # 从最早消息开始消费
            >>> offset = await manager.reset_offset_to_position(queue, "first")
            >>> print(f"从偏移量 {offset} 开始消费所有历史消息")
            >>>
            >>> # 从特定时间戳开始消费
            >>> specific_time = int(time.time() * 1000) - 3600000  # 1小时前
            >>> offset = await manager.reset_offset_to_position(queue, "timestamp", specific_time)
        ...
        return await self.get_consume_offset(queue, strategy, timestamp)
