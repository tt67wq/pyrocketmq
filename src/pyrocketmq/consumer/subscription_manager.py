...
__all__ = ["SubscriptionManager", "AsyncSubscriptionManager"]
...
class AsyncSubscriptionManager:
    ...
    管理Consumer的Topic订阅关系，提供异步的订阅操作。
    支持订阅的增删改查、冲突检测、数据持久化等功能。
    使用异步锁替代线程锁，适用于异步环境。
    """
    ...
    async def asubscribe(self, topic: str, selector: MessageSelector) -> bool:
        """异步订阅Topic
        ...
        async with self._lock:
            ...
            # 检查是否存在冲突
            conflict = await self._acheck_subscription_conflict(topic, selector)
            if conflict:
                self._conflict_history.append(conflict)
                pass  # MVP版本移除指标记录
                raise SubscriptionConflictError(
    ...
    async def aunsubscribe(self, topic: str) -> bool:
        """异步取消订阅Topic
    ...
    async def aupdate_selector(self, topic: str, new_selector: MessageSelector) -> bool:
        """异步更新消息选择器
        ...
        async with self._lock:
            ...
            # 检查冲突
            conflict = await self._acheck_subscription_conflict(topic, new_selector)
            if conflict:
                self._conflict_history.append(conflict)
                pass  # MVP版本移除指标记录
                raise SubscriptionConflictError(
    ...
    async def aget_subscription(self, topic: str) -> SubscriptionEntry | None:
        """异步获取Topic订阅信息
    ...
    async def aget_all_subscriptions(self) -> list[SubscriptionEntry]:
        """异步获取所有订阅信息
    ...
    async def aget_active_subscriptions(self) -> list[SubscriptionEntry]:
        """异步获取所有活跃订阅
    ...
    async def ais_subscribed(self, topic: str) -> bool:
        """异步检查是否已订阅Topic
    ...
    async def aget_subscription_count(self) -> int:
        """异步获取订阅数量
    ...
    async def aget_topics(self) -> set[str]:
        """异步获取所有订阅的Topic列表
    ...
    async def ato_subscription_data_list(self) -> list[SubscriptionData]:
        """异步转换为SubscriptionData列表
    ...
    async def afrom_subscription_data_list(
        self, subscription_data_list: list[SubscriptionData]
    ) -> None:
        """异步从SubscriptionData列表恢复订阅关系
    ...
    async def aexport_subscriptions(self) -> dict[str, Any]:
        """异步导出订阅关系
    ...
    async def aimport_subscriptions(self, data: dict[str, Any]) -> None:
        """异步导入订阅关系
    ...
    async def adeactivate_subscription(self, topic: str) -> bool:
        """异步停用订阅（不删除，只是标记为非活跃）
    ...
    async def aactivate_subscription(self, topic: str) -> bool:
        """异步激活订阅
    ...
    async def acleanup_inactive_subscriptions(
        self, inactive_threshold: timedelta = timedelta(hours=24)
    ) -> int:
        """异步清理长时间非活跃的订阅
    ...
    async def avalidate_subscription(
        self, topic: str, selector: MessageSelector
    ) -> bool:
        """异步验证订阅是否有效
    ...
    async def aget_conflict_history(
        self, limit: int = 100
    ...
    async def aclear_conflict_history(self) -> None:
        """异步清除冲突历史"""
        async with self._lock:
            self._conflict_history.clear()
    ...
    async def _acheck_subscription_conflict(
        self, topic: str, new_selector: MessageSelector
    ) -> SubscriptionConflict | None:
        """异步检查订阅冲突
    ...
    async def _aupdate_metrics(self) -> None:
        """异步更新统计信息（MVP版本简化）"""
        # MVP版本暂不实现复杂指标
        await asyncio.sleep(0)  # 确保异步兼容性

    async def aget_status_summary(self) -> dict[str, Any]:
        """异步获取状态摘要
    ...
    def __str__(self) -> str:
        """返回异步订阅管理器的字符串表示（同步方法）"""
        total = len(self._subscriptions)
        active = len([s for s in self._subscriptions.values() if s.is_active])
        return f"AsyncSubscriptionManager(subscriptions={total}, active={active})"

    def __repr__(self) -> str:
        """返回异步订阅管理器的详细表示（同步方法）"""
        active_count = len([s for s in self._subscriptions.values() if s.is_active])
        return (
            f"AsyncSubscriptionManager(total={len(self._subscriptions)}, "
            f"active={active_count}, "
            f"conflicts={len(self._conflict_history)}, "
            f"max_subscriptions={self._max_subscriptions})"
        )
