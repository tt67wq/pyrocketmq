...
1. 核心生命周期管理模块 (Core Lifecycle Management)
   功能：管理消费者的启动、停止和运行状态
   函数：
     - __init__: 初始化消费者，创建核心组件
     - start: 启动消费者和所有后台任务
     - shutdown: 优雅停止消费者
     - is_running: 检查运行状态
     - get_config: 获取配置
   依赖关系：被所有其他模块使用，是整个系统的基础

2. 订阅管理模块 (Subscription Management)
   功能：管理Topic订阅关系，包括订阅、取消订阅和查询订阅状态
   函数：
     - subscribe: 订阅Topic并注册监听器
     - unsubscribe: 取消订阅
     - get_subscribed_topics: 获取已订阅Topic列表
     - is_subscribed: 检查是否已订阅
   关联：
     - 与"监听器管理模块"紧密配合，共同管理消息监听
     - 被"路由信息管理模块"用于获取需要刷新路由的Topic列表

3. 消息消费处理模块 (Message Consumption)
   功能：处理消息消费的核心逻辑，包括并发消费、顺序消费和重试机制
   函数：
     - _prepare_message_consumption: 消息消费准备（被并发和顺序消费共同使用）
     - _concurrent_consume_message: 并发消费消息
     - _orderly_consume_message: 顺序消费消息
     - check_reconsume_times: 检查重试次数
     - _filter_messages_by_tags: 标签过滤
     - _send_back_message: 消息回发
     - _reset_retry: 重置重试属性
   关联：
     - 依赖"监听器管理模块"获取消息处理器
     - 与"重试主题管理模块"配合实现重试机制
     - 被"监听器管理模块"中的get_consumer_info调用统计消费信息

4. 监听器与状态管理模块 (Listener and Status Management)
   功能：管理消息监听器，提供运行状态查询和监控接口
   函数：
     - get_message_listener: 获取指定Topic的监听器
     - get_all_listeners: 获取所有监听器
     - get_subscription_manager: 获取订阅管理器
     - get_stats_manager: 获取统计管理器
     - get_consume_status: 获取消费状态
     - get_consumer_info: 获取消费者运行信息
     - _on_notify_get_consumer_running_info: 处理运行信息通知
   关联：
     - 被"消息消费处理模块"调用获取监听器
     - 被运维工具调用监控消费者状态
     - 调用"资源清理模块"相关组件获取统计信息

5. 路由信息管理模块 (Route Information Management)
   功能：管理Topic路由信息，包括路由刷新、更新等
   函数：
     - _start_route_refresh_task: 启动路由刷新任务
     - _route_refresh_loop: 路由刷新循环
     - _refresh_all_routes: 刷新所有Topic路由
     - _update_route_info: 更新单个Topic路由
   关联：
     - 被"核心生命周期管理模块"的start方法调用
     - 依赖"订阅管理模块"获取需要刷新的Topic列表
     - 与"心跳管理模块"并行运行，共同维护消费者与集群的连接

6. 心跳管理模块 (Heartbeat Management)
   功能：管理与Broker的心跳连接，维持消费者在线状态
   函数：
     - _start_heartbeat_task: 启动心跳任务
     - _heartbeat_loop: 心跳发送循环
     - _collect_broker_addresses: 收集Broker地址
     - _build_heartbeat_data: 构建心跳数据
     - _send_heartbeat_to_broker: 发送心跳到单个Broker
     - _update_heartbeat_statistics: 更新心跳统计
     - _send_heartbeat_to_all_brokers: 发送心跳到所有Broker
   关联：
     - 被"核心生命周期管理模块"的start方法调用
     - 依赖"路由信息管理模块"获取Broker地址
     - 与"路由信息管理模块"并行运行，共同维护消费者与集群的连接

7. 重试主题管理模块 (Retry Topic Management)
   功能：管理重试主题的订阅和相关逻辑
   函数：
     - _get_retry_topic: 获取重试主题名
     - _is_retry_topic: 判断是否为重试主题
     - _subscribe_retry_topic: 订阅重试主题
   关联：
     - 被"核心生命周期管理模块"的start方法调用（集群模式）
     - 被"消息消费处理模块"使用，实现重试消息路由

8. 资源清理模块 (Resource Cleanup)
   功能：清理消费者资源，包括线程池关闭、组件清理等
   函数：
     - _shutdown_thread_pools: 关闭线程池
     - _cleanup_resources: 清理所有资源
   关联：
     - 被"核心生命周期管理模块"的shutdown方法调用
     - 按依赖顺序清理所有其他模块的资源

9. 工具和辅助方法模块 (Utility Functions)
   功能：提供字符串表示等辅助功能
   函数：
     - __str__: 简洁字符串表示
     - __repr__: 详细字符串表示
...
==================
            ...
            │                   │                   │
    ┌───────▼──────┐    ┌──────▼──────┐    ┌──────▼──────┐
    │ 路由信息管理  │    │ 心跳管理模块  │    │ 重试主题管理  │
    │ (5) Routes   │    │ (6) Heartbeat│    │ (7) Retry   │
    └───────┬──────┘    └──────┬──────┘    └──────┬──────┘
            │                   │                   │
            └───────────┬───────┴───────────────────┘
                        │
...
class BaseConsumer:
    ...
    def get_consumer_info(self) -> ConsumerRunningInfo:
        ...
        running_info.properties["PROP_CLIENT_VERSION"] = "v1.0.0"

        return running_info

    def _on_notify_get_consumer_running_info(
        ...
        if header.client_id == self._config.client_id:
            running_info: ConsumerRunningInfo = self.get_consumer_info()
            return (
