"""
AsyncProcessQueue - 异步高效的消息处理队列

AsyncProcessQueue是ProcessQueue的异步版本，专门为异步RocketMQ Consumer设计，
支持高并发的异步消息处理操作。

主要特性:
- 基于asyncio.Lock的异步线程安全
- 继承ProcessQueue的所有高效数据结构设计
- 异步API支持高并发场景
- 保持墓碑值软删除机制
- 双重缓存限制（数量和大小）
- 支持异步批量操作
...
class AsyncProcessQueue:
    ...
    ProcessQueue的异步版本，支持高并发的异步消息处理。
    继承了ProcessQueue的所有高效数据结构设计和性能特性。
    ...
    async def add_message(self, message: MessageExt) -> bool:
        ...
        async with self._lock:
            ...
            # 检查是否需要清理墓碑值（在插入前清理）
            await self._maybe_cleanup_tombstones()
    ...
    async def add_batch_messages(self, messages: list[MessageExt]) -> dict[str, Any]:
        """
        异步批量添加消息到队列
    ...
    async def remove_message(self, queue_offset: int) -> MessageExt | None:
        """
        异步移除指定偏移量的消息（使用墓碑值软删除，O(1)性能）
    ...
    async def remove_batch_messages(self, queue_offsets: list[int]) -> list[MessageExt]:
        """
        异步批量移除指定偏移量的消息（使用墓碑值软删除）
    ...
    async def _maybe_cleanup_tombstones(self) -> None:
        """异步检查是否需要清理墓碑值，如果需要则执行清理"""
    ...
    async def get_message(self, queue_offset: int) -> MessageExt | None:
        """
        异步获取指定偏移量的消息
    ...
    async def contains_message(self, queue_offset: int) -> bool:
        """异步检查是否包含指定偏移量的消息"""
        async with self._lock:
    ...
    async def get_total_size_mb(self) -> float:
        """异步获取缓存总大小（MB）"""
        async with self._lock:
    ...
    async def get_min_offset(self) -> int | None:
        """异步获取最小偏移量"""
        async with self._lock:
            return None if self._min_offset == float("inf") else int(self._min_offset)

    async def get_max_offset(self) -> int | None:
        """异步获取最大偏移量"""
        async with self._lock:
            return None if self._max_offset == -1 else self._max_offset

    async def clear(self) -> list[MessageExt]:
        """异步清空队列并返回所有有效消息（排除墓碑值）"""
        async with self._lock:
            # 收集所有有效消息（排除墓碑值）
    ...
    def _check_cache_limits(self) -> bool:
        ...
        # 检查大小限制
        if self._total_size >= self._max_cache_size:
            return False
    ...
    async def get_stats(self) -> dict[str, Any]:
        """异步获取统计信息（包含墓碑值统计）"""
        async with self._lock:
            total_size_mb: float = round(self._total_size / (1024 * 1024), 2)
    ...
    async def take_messages(self, batch_size: int) -> list[MessageExt]:
        """
        异步从队列中取出一批消息用于顺序消费处理
    ...
    async def rollback(self, msgs: list[MessageExt] | None = None) -> int:
        """
        异步回滚正在处理的消息，将它们重新放回队列头部
        ...
        if msgs is None:
            return await self._rollback_all()

        # 选择性回滚指定的消息
        return await self._rollback_specific_messages(msgs)

    async def _rollback_all(self) -> int:
        """
        异步回滚所有待提交区的消息
    ...
    async def _rollback_specific_messages(self, msgs: list[MessageExt]) -> int:
        """
        异步回滚待提交区中指定的消息
    ...
    # 同步便捷方法（为了向后兼容，内部调用异步方法）
    async def add_message_sync(self, message: MessageExt) -> bool:
        """同步版本的add_message，为了向后兼容"""
        return await self.add_message(message)

    async def get_message_sync(self, queue_offset: int) -> MessageExt | None:
        """同步版本的get_message，为了向后兼容"""
        return await self.get_message(queue_offset)
