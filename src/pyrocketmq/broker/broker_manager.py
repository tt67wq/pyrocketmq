...
from pyrocketmq.remote.sync_remote import ClientRequestFunc
...
class BrokerManager:
    ...
    _pool_processors: dict[int, ClientRequestFunc]
    ...
    def _create_connection_pool(
        ...
        for request_code, processor_func in self._pool_processors.items():
            pool.register_request_processor(request_code, processor_func)
    ...
    def register_pool_processor(
        self, request_code: int, processor_func: ClientRequestFunc
    ) -> None:
        """注册连接池处理器

        注册一个处理器函数，该函数会自动应用到当前和所有未来创建的连接池。
        ...
        Args:
            ...
            processor_func (ClientRequestFunc): 处理器函数
                - 类型为 Callable[[RemotingCommand, tuple[str, int]], RemotingCommand | None]
                - 接收RemotingCommand和连接地址，返回处理后的RemotingCommand或None
                - 用于处理服务器主动发起的请求或特定类型的响应

        Example:
            >>> def custom_processor(request, addr):
            ...     # 处理自定义请求
            ...     print(f"处理请求 {request.code} 来自 {addr}")
            ...     return None  # 返回None表示不处理
            ...
            >>> manager = BrokerManager(remote_config)
            >>> # 注册处理器，会应用到所有连接池
            >>> manager.register_pool_processor(1001, custom_processor)
            >>>
            ...
            >>> # 为已存在的连接池注册处理器
            >>> manager.add_broker("broker1:10911")
            >>> manager.register_pool_processor(1002, another_processor)
