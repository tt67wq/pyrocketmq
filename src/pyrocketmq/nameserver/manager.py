...
class NameServerManager:
    ...
    def get_broker_address_in_subscription(
        ...
        Example:
            ...
            result = manager.get_broker_address_in_subscription("broker-a", 0)
            if result:
                broker_addr, is_master = result
                print(f"连接到{'master' if is_master else 'slave'}: {broker_addr}")

            # 获取broker地址，建议连接特定slave
            result = manager.get_broker_address_in_subscription("broker-a", 1)
            ```
        ...
        topic_route_data: TopicRouteData | None = (
            self._query_topic_route_from_nameserver(broker_name)
        )
    ...
    def _parse_nameserver_addrs(self, nameserver_addrs: str) -> list[str]:
        ...
        """
        addrs: list[str] = [
            addr.strip() for addr in nameserver_addrs.split(";") if addr.strip()
        ]
        return addrs
...
def create_nameserver_manager(
    ...
    Note:
        ...
        使用完毕后应调用stop()方法清理资源。
    """
    config: NameServerConfig = NameServerConfig(
        nameserver_addrs=nameserver_addrs, **kwargs
    )
